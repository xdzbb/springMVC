...
public class ArticleController{
	...
	public String saveArticle(Article article,String vdcode,@RequestParam("ArtContent") String content,ModelMap map,HttpSession session){
		String Session_ValidateCode=(String) session.getAttribute("Session_ValidateCode");
		String error="";
		//验证码
		if(Session_ValidateCode==null||"".equals(Session_ValidateCode)||!Session_ValidateCode.equalsIgnoreCase(vdcode)){
			error = "验证码错误！";
		}else if(article.getTypeid().equals("")){
			error = "请选择分类！";
		}else if(article.getUserid().equals("")){
			error = "登录用户才能发表文章！";
		}		
